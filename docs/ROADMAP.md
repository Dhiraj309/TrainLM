...
This is an execution plan, not a list of possible ideas. Only one milestone
should be active at a time. Every numbered feature/story is one future atomic
commit containing its implementation, tests, and narrowly required
documentation.

Status flags: `[ ]` not started, `[~]` implementation in progress or awaiting
validation, `[x]` fully complete, and `[d]` intentionally deferred.

Three rules protect the product goal:

1. Generic Hugging Face compatibility is proven before optimized adapters.
2. Universal dense-AR TPU compatibility is proven before Llama-focused parity
   work dominates development.
3. Performance claims require repeatable real-TPU evidence; a fallback that
   merely runs is never called optimized.
...
### Hugging Face remains the model and checkpoint contract
...
### TrainLM owns the performance-sensitive training engine
...
### Support means more than execution
...
| [ ] | M4 | Correct TrainLM trainer on CPU/CUDA | Blocking |
| [ ] | M5 | Stable PyTorch/XLA DP8 runtime and accumulation decision | Blocking |
| [ ] | M6 | Universal dense-AR TPU compatibility before Llama optimization | Blocking |
| [ ] | M7 | Production checkpointing, observability, and integrity | High |
| [ ] | M8 | Capability planner and reversible optimization engine | Blocking |
| [ ] | M9 | Memory-efficient causal-LM loss | Performance |
| [ ] | M10 | TPU attention provider family and 850K gate | Performance |
| [ ] | M11 | Projection, optimizer, remat, and HLO parity closure | Performance |
| [ ] | M12 | Exact LaughLM 135M parity certification | Release gate |
| [ ] | M13 | Cross-family optimization and 1.3B scaling | Framework gate |
| [ ] | M14 | Dense-AR V1 production release | Final |
| [d] | M15 | TorchTPU backend migration | Future |
| [d] | M16 | MoE extension | Future |
| [d] | M17 | DLLM extension | Future |

## Milestones and atomic feature commits
...
- [ ] scope, parity data, metric formulas, and dependency profiles are
...
- [ ] CPU contract tests demonstrate that trainer, task, optimization, and
...
- [ ] all representative dense AR families satisfy Level 1 compatibility on
...
- [ ] the 28/2 LaughLM shard split can stream deterministic fixed-shape
...
- [ ] the trainer is correct on CPU and has a CUDA smoke path, while the
...
- [ ] PyTorch/XLA is a functional replaceable backend, DP8 is correct, and
...
- [ ] every V1 capability cluster trains generically on TPU, does not
...
- [ ] 200 updates can survive checkpoint/resume, produce trustworthy metrics,
...
- [ ] no-op and fixture transformations are transactional, explainable,
...
- [ ] the reference geometry no longer materializes full logits, gradient
...
- [ ] MHA/GQA/MQA have correct optimized providers, positional/mask variants
...
- [ ] transformations are reversible and correct, the final graph clears
...
- [ ] exact 135M parity is independently reproducible and clears the hard
...
- [ ] all advertised dense-AR capability clusters are Level 3 or explicitly
...
- [ ] a new user can securely reproduce a certified v5e-8 run and export a
...
### [d] M15-F1 — TorchTPU compatibility ADR
...
### [d] M15-F2 — Native TorchTPU execution backend
...
### [d] M15-F3 — TorchTPU Pallas kernel providers
...
### [d] M15-F4 — TorchTPU correctness and checkpoint parity
...
### [d] M15-F5 — TorchTPU performance parity
...
- [d] TorchTPU meets or beats the accepted PyTorch/XLA correctness and
...
### [d] M16-F1 — MoE capability and task contract
...
### [d] M16-F2 — Grouped/ragged expert kernels
...
### [d] M16-F4 — MoE checkpoint and resume
...
### [d] M16-F5 — MoE certification matrix
...
- [d] representative HF MoE causal LMs are Certified; dense regression
...
### [d] M17-F1 — Diffusion LM task protocol
...
### [d] M17-F2 — Diffusion attention and mask capabilities
...
### [d] M17-F4 — Compiled denoising training step
...
### [d] M17-F5 — DLLM certification matrix
...
- [d] representative DLLMs are Certified under a distinct task contract;
...
## Reference implementations and API decisions
