...
It is the authority for scope decisions, issue classification, tests, release
notes, and public performance claims.

The roadmap explains when capabilities will be delivered. This contract
explains which models and behaviors TrainLM promises to cover. If another
document conflicts with this contract, this document controls until a reviewed
scope change updates both it and the roadmap.

## Product promise

TrainLM will accept an ordinary in-scope Hugging Face
`AutoModelForCausalLM`, train it through TrainLM's own performance-sensitive
...
family adapters may describe model structure, but the trainer, task, data,
checkpoint, and runtime layers must remain model-family independent.
...
- it returns a causal-LM loss when given labels, or returns logits from which
  the standard shifted causal loss can be computed;
- its trainable blocks are dense Transformer blocks rather than routed experts;
- its required operations have PyTorch semantics that the selected TrainLM
  backend can compile or execute correctly;
- it does not require a CUDA-only operation for its standard training path;
- it can be saved to, and reconstructed from, a canonical Hugging Face
  configuration and state dictionary.

This is a structural contract, not a list of model names. A new model family
that satisfies the contract is eligible for the generic path even before it has
...
- separate Q/K/V, packed QKV, and partially packed projection layouts;
- bias and bias-free projections where the source model defines them;
- fixed-shape data-parallel training on TPU v5e-8;
- FSDP as the larger-model extension within dense-AR V1.

### Training lifecycle

An in-scope model reaches generic compatibility only if it can complete:
...
the entire autoregressive ecosystem.
...
row; certification remains versioned and model-specific.
...
The optimization plan must be explainable. Every unsupported operation must be
reported as a fallback; partial optimization must not be presented as full
certification.
...
- a model-specific performance target defined before the certification run.

Only Certified models may be described as “fully supported,” “optimally
trainable,” or as meeting a published TPU performance target.

## Fallback and strict-mode policy

- Generic fallback is a required compatibility mechanism, not evidence of
  optimization.
- `auto` mode may select a safe generic provider, but must record why a faster
  provider was not selected.
- `required` or certification mode must fail before TPU allocation when a
  requested capability cannot be provided safely.
...
- `trainer.explain()` is the public source for selected providers,
  transformations, fallbacks, and certification state.
...
A remote-code model may be evaluated only through an explicit user opt-in. It
is best-effort and uncertified until TrainLM records the exact repository
revision, audits its training behavior, supplies an adapter only where needed,
and passes the same conformance and certification gates as built-in models.
...
MoE and diffusion language models have future roadmap milestones. Supporting
them later must extend the task/capability contracts without weakening dense-AR
correctness or performance gates.

## Issue classification

Issues that change or question this contract use the `scope` label plus one of
...
A model request must identify the HF model/config revision, architecture type,
desired support level, backend/hardware, and any custom-code requirement.

## Release-note requirements

Every release that changes model support must state:

- the affected model family and configuration/revision range;
...
- a link to benchmark evidence for any performance claim.

Release notes must not use Compatible, Optimized, and Certified
interchangeably. The generated-release category labels are defined in
`.github/release.yml`.
...
2. updates this contract and the roadmap;
